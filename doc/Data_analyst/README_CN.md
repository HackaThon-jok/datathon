...
负责 KPI 业务语义、独立业务基准、看板要求及业务签核。如果被指定为 Streamlit 负责人，必须交付可运行的 Python 代码；只有 Wireframe 不算完成看板。
...
- 独立于候选转换计算并冻结源／旧系统基准。
- 定义 Streamlit 字段、筛选、更新时间、验证状态及无数据／错误行为。
...
- 编写或协助实现 App Runner 上的 Python Streamlit，并提供月份／地区筛选。
